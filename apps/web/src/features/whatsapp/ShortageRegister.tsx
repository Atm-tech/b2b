...
type Line = { sku: string; name: string; requested: number; available: number; pending: number; released: number; purchaseQuantity: number; cancelled: number };
type ShortageCase = { id: string; draft_id: string; retailer_name: string; salesman_name: string; purchaser_name?: string; retailer_choice: string; purchase_status: string; purchase_order_id?: string; status: string; warehouse_id: string; created_at: string; next_action_at?: string; expected_at?: string; decision_note: string; balance_draft_id?: string; available_sales_cart_id?: string; balance_sales_cart_id?: string; supply_status: string; supply_review_required: boolean; portions: Array<{draftId: string; status: string; salesCartId?: string}>; receipts: Array<{sku: string; ordered: number; received: number; outstanding: number; status: string}>; failed_notifications: number; lines: Line[]; events: Array<{ id: number; action: string; actor: string; note: string; created_at: string }> };
...
function ShortageCard({ item, register, snapshot, busy, onAction }: { item: ShortageCase; register: Register; snapshot: AppSnapshot; busy: boolean; onAction: (id: string, action: string, body: Record<string, unknown>) => Promise<void> }) {
  ...
  return <article className="panel shortage-card">
    ...
    <div className="table-wrap"><table><thead><tr><th>Product</th><th>Requested</th><th>Available portion</th><th>Pending balance</th><th>Released balance</th><th>Cancelled</th></tr></thead><tbody>{item.lines.map(line => <tr key={line.sku}><td><strong>{line.name}</strong><small>{line.sku}</small></td><td>{line.requested}</td><td>{line.available}</td><td>{line.pending}</td><td>{line.released}</td><td>{line.cancelled}</td></tr>)}</tbody></table></div>
    ...
    {item.receipts?.length ? <details open={item.supply_review_required}><summary>Supplier delivery: {item.supply_status || "Awaiting receipt"}</summary>
      <div className="table-wrap"><table><thead><tr><th>Product</th><th>PO quantity</th><th>Received</th><th>Supplier balance</th><th>Receipt status</th></tr></thead><tbody>{item.receipts.map(receipt => <tr key={receipt.sku}><td>{receipt.sku}</td><td>{receipt.ordered}</td><td>{receipt.received}</td><td>{receipt.outstanding}</td><td>{receipt.status}</td></tr>)}</tbody></table></div>
      <p className="helper-text">Received stock must pass warehouse review before it can be released. Supplier balance and retailer pending balance are tracked separately.</p>
    </details> : null}
    {item.portions?.length ? <details><summary>Replenishment portions ({item.portions.length})</summary><ul>{item.portions.map(portion => <li key={portion.draftId}>{portion.draftId} / {portion.status} / {portion.salesCartId || "Retailer confirmation pending"}</li>)}</ul></details> : null}
    {register.canManage && ["Approved","Cancelled"].includes(item.purchase_status) && item.retailer_choice !== "Cancel Balance" && item.lines.some(line => line.pending > 0) ? <details open={item.supply_review_required}><summary>Supplier delay or partial receipt: Sales decision</summary><div className="form-grid">
      <p className="helper-text wide-field">Record the retailer agreement. Existing confirmation portions stay active; this decision applies to the remaining balance. Cancelling retailer demand does not cancel the supplier PO.</p>
      <label className="wide-field">Retailer agreement<input value={note} onChange={e => setNote(e.target.value)} /></label>
      <label>Revised expected arrival<input type="datetime-local" value={expectedAt} onChange={e => setExpectedAt(e.target.value)} /></label>
      <button className="ghost-button" disabled={busy || !note.trim() || !expectedAt} onClick={() => void onAction(item.id,"supply",{decision:"Wait",expectedAt:new Date(expectedAt).toISOString(),note})}>Wait until revised date</button>
      <button className="primary-button" disabled={busy || !note.trim()} onClick={() => void onAction(item.id,"supply",{decision:"Dispatch",note})}>Confirm available stock; keep balance pending</button>
      <button className="ghost-button" disabled={busy || !note.trim()} onClick={() => void onAction(item.id,"supply",{decision:"Cancel",note})}>Cancel unallocated retailer balance</button>
    </div></details> : null}
...
export function ShortageRegister({ snapshot, sessionToken }: { snapshot: AppSnapshot; sessionToken: string }) {
  ...
  return <div className="shortage-register"><Panel title="Open shortage register" eyebrow="Sales ownership  /  Purchase approval  /  Admin oversight">
    ...
    <p className="helper-text">Supplier delays and partial receipts require Sales follow-up. All unresolved cases remain here regardless of order date. Available orders, pending quantities and purchase decisions stay linked until fulfilment and payment are complete.</p>
