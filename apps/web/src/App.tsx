...
import { VoiceTrainingView } from "./features/assistant/VoiceTrainingView";
...
function App() {
  ...
  async function doLogout() {
    ...
    setSessionToken("");
    setSnapshot(null);
    setSalesForm((current) => ({ ...current, shopId: "", billingType: "B2C" }));
    ...
    <section className="collapse-stack">
      ...
      <CollapsiblePanel title={`Update ${partyRoleLabel}`} eyebrow="Edit details" open={openPartyPanel === "update"} onToggle={() => setOpenPartyPanel((current) => current === "update" ? "" : "update")}>
        <form className="form-grid" onSubmit={(e) => { e.preventDefault(); void patch(`/counterparties/${partyEditForm.id}`, partyEditForm, "Party updated.", () => setPartyEditForm(emptyPartyEditForm)); }}>
          ...
          {partyEditForm.type === "Shop" ? <><label className="checkbox-line"><input type="checkbox" checked={partyEditForm.allowLaterCollection} onChange={(e) => setPartyEditForm((c) => ({ ...c, allowLaterCollection: e.target.checked }))} />Allow collect later</label><label className="checkbox-line"><input type="checkbox" checked={partyEditForm.allowPartialCollection} onChange={(e) => setPartyEditForm((c) => ({ ...c, allowPartialCollection: e.target.checked }))} />Allow partial collection</label><label className="checkbox-line"><input type="checkbox" checked={partyEditForm.allowChequeCollection} onChange={(e) => setPartyEditForm((c) => ({ ...c, allowChequeCollection: e.target.checked }))} />Allow cheque collection</label><label>Collection tolerance<input type="number" min="0" step="0.01" value={partyEditForm.collectionTolerance} onChange={(e) => setPartyEditForm((c) => ({ ...c, collectionTolerance: e.target.value }))} /></label></> : null}
    ...
    <main className={`${effectiveSimpleMode ? "app-shell simple-shell" : "app-shell"}${isWhatsAppWorkspaceUser ? " whatsapp-admin-app" : ""}`}>
      ...
      <section className={effectiveSimpleMode ? "workspace-shell simple-workspace" : `workspace-shell${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
        ...
        <div className="content-shell">
          ...
          {activeView === "Delivery" || activeView === "CurrentDelivery" || activeView === "NewAssignment" ? (
            isAdminUser ? (
              ...
              <DeliveryJobsView
                ...
                onUpdateTask={(id, body) => patch(`/delivery-tasks/${id}`, body, "Delivery task updated.")}
                onCreatePayment={(body) => post("/payments", body, "Collection recorded.")}
              />
            ) : isDeliveryManager ? (
              ...
              <WarehouseOperationsViewV2
                snapshot={snapshot}
                currentUser={currentUser}
                onUploadProof={async (file) => uploadFile("/receipt-checks/upload-proof", "receiptProof", file, "Weight proof uploaded.")}
                onUploadPaymentProof={async (file) => uploadFile("/payments/upload-proof", "proof", file, "Payment proof uploaded.")}
                onReceive={(body) => post("/receipt-checks", body, "Warehouse receipt saved.")}
                onUpdateTask={(id, body) => patch(`/delivery-tasks/${id}`, body, "Delivery task updated.")}
                onUpdateSalesOrder={(id, body) => patch(`/sales-orders/${id}`, body, "Sales bill updated.")}
                onCreateDockets={(body) => post("/delivery-dockets", body, "DCO docket created.")}
                onCreateDeliveryTask={(body) => post("/delivery-tasks", body, "Delivery task assigned.")}
                onMergeDeliveryTasks={(body) => post("/delivery-tasks/merge", body, "Outbound deliveries merged.")}
                onCreateConsignment={(body) => post("/delivery-consignments", body, "DCO created and queued for delivery.")}
                screen="out"
                canManageDeliveryTagging={true}
          ...
          {activeView === "VoiceTrainer" ? <VoiceTrainingView sessionToken={sessionToken} onMessage={setMessage} onError={setError} /> : null}
