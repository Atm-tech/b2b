...
import {createReturnReceiptMethods,canReceiveReturn,requireAgentReturnPhotos} from './delivery-return-receipts.js';
import {applyDeliveryAmendment,deliveryAmendment} from './delivery-exception-billing.js';
...
export async function assertDeliveryExceptionUpdate(db:Db,task:any,payload:any){
  const cases=(await db.query("SELECT * FROM delivery_exceptions WHERE task_id=$1 AND status NOT IN ('Retry Scheduled','Withdrawn') AND warehouse_received_at IS NULL",[task.id])).rows;
  if(!cases.length)return;
  if(payload.status==='Delivered'&&cases.every(r=>r.bill_adjusted_at))payload.status='Handed Over';
  ...
  for(const row of cases){const before=stops.find((s:any)=>s.orderId===row.order_id);const after=payload.routeStops?.find((s:any)=>s.orderId===row.order_id);
    if(row.bill_adjusted_at&&after){
      const collectionKeys=['paid','picked','collectionStatus','collectionAmount','collectionMode','collectionReference','collectionProofName','cashDenominations','paymentRequired'];
      const fixed=(s:any)=>Object.fromEntries(Object.entries(s).filter(([key])=>!collectionKeys.includes(key)));
      if(!isDeepStrictEqual(fixed(before),fixed(after)))throw Error('The adjusted delivery stop changed. Refresh the approved bill before continuing.');
      if(after.collectionStatus==='Later')throw Error('Collect the adjusted amount at this delivery stop.');
      // Collection state comes from recorded payments, never client-supplied totals.
      for(const key of collectionKeys){if(before[key]===undefined)delete after[key];else after[key]=before[key];}
    }else if(!after||!isDeepStrictEqual(before,after))throw Error('This stop has a delivery exception. Submit the report and follow the seller decision before changing delivery or collection.');
  }
}
export function createDeliveryExceptionService(deps:Deps){
  ...
  async function decide(id:string,input:{decision:string;note:string;dueAt?:string;revision:number},a:Actor,admin=false){return deps.transaction(async db=>{const {row,task}=await load(db,id);seller(row,a,admin);if(row.decision_revision===input.revision&&row.decision===input.decision)return row;version(row,input.revision);if(row.status!=='Awaiting Seller'&&!(row.status==='Return Authorized'&&!row.bill_adjusted_at&&input.decision==='Return'))throw Error('This report is not awaiting a seller decision.');if(!input.note?.trim())throw Error('Record the seller decision reason.');
    ...
    if(input.decision!=='Correction')await requireAgentReturnPhotos(db,row);
    if(input.decision==='Return')await applyDeliveryAmendment(db,row,task);
    if(input.decision==='Retry')await db.query("UPDATE delivery_tasks SET route_json=(SELECT jsonb_agg(CASE WHEN s->>'orderId'=$2 THEN s-'deliveryExceptionId' ELSE s END) FROM jsonb_array_elements(route_json) s) WHERE id=$1",[task.id,row.order_id]);
    const next=(await db.query('UPDATE delivery_exceptions SET status=$2,decision=$3,decision_note=$4,retry_at=$5,decision_revision=revision,revision=revision+1,updated_at=NOW() WHERE id=$1 RETURNING *',[id,status,input.decision,input.note,input.dueAt||null])).rows[0];await audit(db,id,'Seller decision',a,`${input.decision}: ${input.note}${next.bill_adjusted_at?`; revised bill Rs.${Number(next.adjusted_total).toFixed(2)}; returned goods remain with agent`:''}`);await notify(db,next);return next;
  });}
  async function withdraw(id:string,v:number,a:Actor,admin=false){return deps.transaction(async db=>{const {row}=await load(db,id);agent(row,a,admin);version(row,v);if(!editable(row))throw Error('A submitted report needs a seller decision.');const next=(await db.query("UPDATE delivery_exceptions SET status='Withdrawn',revision=revision+1,updated_at=NOW() WHERE id=$1 RETURNING *",[id])).rows[0];await db.query("UPDATE delivery_tasks SET route_json=(SELECT jsonb_agg(CASE WHEN s->>'orderId'=$2 THEN s-'deliveryExceptionId' ELSE s END) FROM jsonb_array_elements(route_json) s) WHERE id=$1",[row.task_id,row.order_id]);await audit(db,id,'Draft withdrawn',a);return next;});}
  async function list(a:Actor,admin=false){const allowed=admin||['Sales','Delivery','Out Delivery','Warehouse Manager'].some(r=>has(a,r));if(!allowed)throw Error('Delivery exception access required.');const cases=(await deps.query(`SELECT e.*,c.name AS retailer_name,u.full_name AS seller_name,
    COALESCE((SELECT SUM(p.amount) FROM payments p WHERE p.side='Sales' AND p.linked_order_id=e.order_id AND p.verification_status IN ('Submitted','Verified','Resolved')),0) AS paid_amount,
    COALESCE((SELECT jsonb_agg(jsonb_build_object('id',p.id,'stage',p.stage,'lineId',p.line_id,'createdBy',p.created_by_name,'createdAt',p.created_at,'sha256',p.sha256) ORDER BY p.created_at) FROM delivery_exception_photos p WHERE p.case_id=e.id),'[]') AS photos,
    ...
    return {cases:cases.map(r=>({...r,canReceive:canReceiveReturn(a,r),proposed_bill:deliveryAmendment(r),amount_due:r.bill_adjusted_at?Math.max(0,Math.round((Number(r.adjusted_total)-Number(r.paid_amount))*100)/100):null,canEdit:admin||(['Delivery','Out Delivery'].some(role=>has(a,role))&&r.agent_username.toLowerCase()===a.username.toLowerCase()),canDecide:admin||(has(a,'Sales')&&Number(r.salesman_id)===a.id)}))};}
