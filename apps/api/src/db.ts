...
async function syncWhatsAppCatalogMinimums() {
  ...
  const mrps = whatsappCatalogMinimums.map((item) => whatsappCatalogMrps[item.articleName]);
  ...
  const updated = await pool.query<{ sku: string }>(
    `WITH source AS (
       SELECT article_name, minimum_quantity, sale_price, mrp, product_sku
       FROM UNNEST($1::text[], $2::double precision[], $3::double precision[], $4::double precision[], $5::text[]) AS source_values(article_name, minimum_quantity, sale_price, mrp, product_sku)
     )
     ...
     RETURNING product.sku`,
      [names, quantities, salePrices, mrps, aliases]
  );
