...
export async function assertNoPackingHold(db:Pick<PoolClient,'query'>,ids:string[]) {
  ...
  for(const row of rows){
    ...
    if((await db.query("SELECT 1 FROM delivery_exceptions WHERE order_id=$1 AND status NOT IN ('Retry Scheduled','Withdrawn')",[row.cart])).rowCount)throw new Error('This order has an unresolved delivery exception. Follow the seller decision and goods return process before changing it.');
