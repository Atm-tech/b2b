...
export function createRetailerFinanceService(deps:FinanceDeps){
 ...
 async function authorize(db:Db,shop:string,a:Actor,admin:boolean){if(admin||has(a,'Accounts'))return;const own=await db.query('SELECT 1 FROM counterparties c WHERE c.id=$1 AND (EXISTS(SELECT 1 FROM whatsapp_retailers r WHERE r.counterparty_id=c.id AND r.salesman_id=$2) OR (NOT EXISTS(SELECT 1 FROM whatsapp_retailers r WHERE r.counterparty_id=c.id) AND EXISTS(SELECT 1 FROM sales_orders o WHERE o.shop_id=c.id AND o.salesman_id=$2)))',[shop,a.id]);if(!has(a,'Sales')||!own.rowCount)throw Error('This retailer belongs to another staff member.');}
 async function list(a:Actor,admin=false){return deps.transaction(async db=>{
  const staff=admin||has(a,'Accounts')||has(a,'Sales');
  const shops=(await db.query(`SELECT c.id,c.name,COALESCE(r.salesman_id,(SELECT MIN(o.salesman_id) FROM sales_orders o WHERE o.shop_id=c.id)) AS salesman_id,($1 OR r.salesman_id=$2 OR (r.counterparty_id IS NULL AND EXISTS(SELECT 1 FROM sales_orders o WHERE o.shop_id=c.id AND o.salesman_id=$2))) AS finance_access
 FROM counterparties c LEFT JOIN whatsapp_retailers r ON r.counterparty_id=c.id
 WHERE (c.type='Shop' OR EXISTS(SELECT 1 FROM sales_orders o WHERE o.shop_id=c.id)) AND (
 $1 OR ($4 AND (r.salesman_id=$2 OR (r.counterparty_id IS NULL AND EXISTS(SELECT 1 FROM sales_orders o WHERE o.shop_id=c.id AND o.salesman_id=$2))))
 OR EXISTS(SELECT 1 FROM retailer_collection_followups f WHERE f.shop_id=c.id AND (f.collector_username=$3 OR ($4 AND f.owner_id=$2)) AND f.status<>'Closed'))`,[admin||has(a,'Accounts'),a.id,a.username,has(a,'Sales')])).rows;
  const ids=shops.map(s=>s.id);const financeIds=shops.filter(s=>s.finance_access).map(s=>s.id);
  return {shops,credits:staff?(await db.query('SELECT * FROM retailer_credit_balances WHERE shop_id=ANY($1::text[]) AND amount>0 ORDER BY updated_at',[financeIds])).rows:[],refunds:staff?(await db.query('SELECT * FROM retailer_refunds WHERE shop_id=ANY($1::text[]) ORDER BY created_at DESC',[financeIds])).rows:[],collections:(await db.query("SELECT f.*,u.full_name AS owner_name,c.name AS shop_name,f.due_at<NOW() AS overdue FROM retailer_collection_followups f LEFT JOIN users u ON u.id=f.owner_id JOIN counterparties c ON c.id=f.shop_id WHERE f.shop_id=ANY($1::text[]) AND f.status<>'Closed' AND ($2 OR f.collector_username=$3 OR f.owner_id=$4 OR EXISTS(SELECT 1 FROM whatsapp_retailers r WHERE r.counterparty_id=f.shop_id AND r.salesman_id=$4) OR (NOT EXISTS(SELECT 1 FROM whatsapp_retailers r WHERE r.counterparty_id=f.shop_id) AND EXISTS(SELECT 1 FROM sales_orders o WHERE o.shop_id=f.shop_id AND o.salesman_id=$4))) ORDER BY f.due_at",[ids,admin||has(a,'Accounts'),a.username,a.id])).rows,staff:staff?(await db.query("SELECT id,username,full_name,role,roles_json AS roles FROM users WHERE active AND (role=ANY($1::text[]) OR roles_json ?| $1::text[])",[['Sales','Collection Agent','Delivery','Out Delivery','Accounts','Admin']])).rows:[]};
 });}
