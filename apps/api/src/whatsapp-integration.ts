...
import { WHATSAPP_COLLECTION_TOLERANCE, paymentOcrAmount } from "./whatsapp-collection-utils.js";
...
async function sendDcoCheckbox(phone: string, user: StaffUser, orders: Array<{ id: string; title: string; description: string }>) {
  if (!orders.length) { await sendText(phone, "Koi packed SO available nahi hai."); return; }
  let flowId: string;
  try { flowId = await ensureDcoCheckboxFlow(); }
  catch {
    await sendGraphMessage(phone, { type: "interactive", interactive: { type: "list", body: { text: `Checkbox form abhi Meta par available nahi hai. Filhal ek SO select karein; Add another se aur SO jod sakte hain.` }, action: { button: "Packed SO", sections: [{ title: "Ready for DCO", rows: orders.slice(0, 10).map((order) => ({ id: `wa-dco:add:${encodeURIComponent(order.id)}`, title: compact(order.title, 24), description: compact(order.description, 72) })) }] } } }, "DCOBuild");
    return;
  }
  ...
  await executeDatabaseQuery("INSERT INTO settings (key,value_json) VALUES ($1,$2::jsonb)", [token, JSON.stringify({ userId: String(user.id), testMode: false, allowed: options.map((order) => order.id), expiresAt: Date.now() + 30 * 60 * 1000 })]);
  await sendGraphMessage(phone, { type: "interactive", interactive: { type: "flow", body: { text: `Packed SO ke checkboxes tick karke Create DCO karein.${orders.length > 20 ? " Pehle 20 ready SO dikhaye gaye hain." : ""}` }, action: { name: "flow", parameters: { flow_message_version: "3", flow_token: token, flow_id: flowId, flow_cta: "Select SOs", flow_action: "navigate", flow_action_payload: { screen: "SELECT_SO", data: { orders: options, heading: `Ek DCO ke liye multiple packed SO select karein.` } } } } } }, "DCOBuild");
}
  ...
  const result = await executeDatabaseQuery<{ id: string }>(
    `SELECT id FROM whatsapp_order_drafts
     WHERE counterparty_id=$1 AND source IN ('Catalogue','Retailer cart')
       ...
       AND NOT EXISTS (SELECT 1 FROM whatsapp_shortage_cases sc WHERE sc.draft_id=whatsapp_order_drafts.id OR sc.id IN (SELECT case_id FROM whatsapp_shortage_portions WHERE draft_id=whatsapp_order_drafts.id))
...
async function sendDraftChangeProductPicker(profile: RetailerProfile, draftId: string) {
  if ((await executeDatabaseQuery("SELECT id FROM whatsapp_shortage_cases WHERE draft_id=$1 OR id IN (SELECT case_id FROM whatsapp_shortage_portions WHERE draft_id=$1)", [draftId])).rowCount) { await sendText(profile.phoneE164, "This order has tracked pending quantities. Please contact your salesperson to change the order."); return; }
  const loaded = await loadDraft(draftId);
...
async function sendDraftEditOptions(profile: RetailerProfile, draftId: string) {
  ...
  const loaded = await loadDraft(draftId);
  if (text(loaded.draft.counterparty_id) !== profile.counterpartyId) throw new Error("This proforma does not belong to your retailer account.");
  if(!["Needs Review","Change Requested","Staff Approved","Awaiting Retailer"].includes(text(loaded.draft.status)))throw new Error("This confirmation is no longer editable. Contact Sales for a new order.");
  await sendButtons(profile.phoneE164,
...
const collectionConfirmations = new Map<string, string>();
...
function deliveryTaskAllowed(task: { assignedTo: string } | undefined, user: StaffUser) {
...
// WhatsApp-only retailers are intentionally excluded from the main app snapshot.
// Read saved privileges directly for both chat options and collection validation.
async function getCollectionRetailer(counterpartyId: string | undefined) {
  if (!counterpartyId) return undefined;
  const result = await executeDatabaseQuery<{
    mobile_number: string; allow_later_collection: boolean;
    allow_partial_collection: boolean; allow_cheque_collection: boolean;
    collection_tolerance: number;
  }>(`SELECT mobile_number, allow_later_collection, allow_partial_collection,
             allow_cheque_collection, collection_tolerance
      FROM counterparties WHERE id=$1 AND type='Shop'`, [counterpartyId]);
  const row = result.rows[0];
  return row ? {
    ...
    allowPartialCollection: Boolean(row.allow_partial_collection),
    allowChequeCollection: Boolean(row.allow_cheque_collection),
    collectionTolerance: Number(row.collection_tolerance)
  } : undefined;
}
...
async function notifyDeliveryAllDone(phone: string, user: StaffUser) {
  const snapshot = await getSnapshot();
  const assigned = snapshot.deliveryTasks.filter((task) => task.side === "Sales" && task.assignedTo.toLowerCase() === user.username.toLowerCase());
  if (assigned.length > 0 && assigned.every((task) => task.status === "Delivered" && task.routeStops.every((stop) => stop.delivered))) {
    await sendText(phone, "*Done for day.* Aaj ke sab assigned DCO delivery stops complete ho gaye hain. Collection total upar diya gaya hai.", "DeliveryComplete");
  }
}
...
async function sendStaffHelp(phone: string, user: StaffUser) {
  ...
  if (delivery) lines.push("Delivery: LIST (DCO/retailer select), TOTAL / SUM, LIST COLLECTION, SETTLE. Buttons se delivery aur collection complete karein.");
...
async function handleStaffWhatsAppMessage(message: JsonObject, from: string, user: StaffUser): Promise<boolean> {
  ...
  if (incomingAction.startsWith("wa-test:") || incomingAction.startsWith("wa-test-delivery:")) {
    await sendText(from, "Purana test flow remove ho gaya hai. Actual orders ke liye SO / DCO / LIST type karein.");
    return true;
  }
  ...
  if (flowReply?.response_json) {
    ...
    if (response.kind === "dco_checkbox" || text(response.flow_token).startsWith("dco-checkbox-")) {
      ...
      if (!data || data.expiresAt < Date.now() || data.testMode) { await sendText(from, "Form expired ya already submitted hai. DCO se fresh form kholein."); return true; }
      let selected: string[];
      ...
      dcoBuildSessions.set(from, selected);
      message = { type: "interactive", interactive: { button_reply: { id: "wa-dco:create" } } };
    }
  }
  ...
  if (["image", "document"].includes(messageType)) {
    ...
    if (pending) {
      ...
      if (task && stop) {
        ...
        await sendButtons(from, `${stop.supplierName} delivery photo saved. Collection amount: Rs.${collectionRemaining(stop).toFixed(2)}`, buttons, "Delivery", task.id);
        return true;
      }
    }
    ...
    if (paymentPending) {
      ...
      if (task && stop) {
        ...
        if (reading?.visible) {
          ...
          const chequePayeeMismatch = paymentPending.mode === "Cheque" && !reading.payeeName.toLowerCase().includes("aapoorti");
          if (reading.amount > collectionRemaining(stop) + tolerance || (paymentPending.kind === "full" && Math.abs(reading.amount - collectionRemaining(stop)) > tolerance) || (reading.amount + tolerance < collectionRemaining(stop) && !party?.allowPartialCollection) || chequePayeeMismatch) {
            await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), reading.amount, chequePayeeMismatch ? `Cheque payee mismatch: ${reading.payeeName}` : `${paymentPending.mode} proof amount mismatch`);
            paymentProofPending.set(from, paymentPending);
            await sendText(from, chequePayeeMismatch ? `Cheque payee *${reading.payeeName}* read hua; Aapoorti ke naam par cheque required hai. WhatsApp Admin ko alert bhej diya gaya hai; Contact Admin.` : `${paymentPending.mode} proof mein Rs.${reading.amount.toFixed(2)} read hua, bill Rs.${collectionRemaining(stop).toFixed(2)} hai. WhatsApp Admin ko alert bhej diya gaya hai; Contact Admin ya clear proof dobara bhejein.`, "Collection", task.id);
            return true;
          }
          collectionConfirmations.set(from, `wa-proof:confirm:${paymentPending.mode.toLowerCase()}:${paymentPending.kind}:${task.id}:${paymentPending.stopIndex}:${reading.amount}`);
        ...
        paymentProofPending.set(from, paymentPending);
        await sendText(from, `${paymentPending.mode} photo se amount clearly read nahi hua. Clear payment photo dobara bhejein.`, "Collection", task.id);
        return true;
      }
    }
  ...
  if (action === "wa-dco:list") {
    ...
    await sendDcoCheckbox(from, user, rows.map(([cartId, lines]) => ({ id: cartId, title: `SO ${shortId(cartId)}`, description: `${lines[0].shopName} - ${lines.map((line) => `${line.productSku} x ${line.quantity}`).join(", ")}` })));
    return true;
  }
  ...
  if (action.startsWith("wa-delivery:stop:")) {
    ...
    if (stop.delivered && stop.paymentRequired && ["Pending", "Later"].includes(stop.collectionStatus || "")) {
      ...
      const choices = privileged?.allowLaterCollection && !stop.deliveryExceptionId ? [{ id: `wa-collect:later:${task.id}:${stopIndex}`, title: "Collect later" }, { id: `wa-collect:now:${task.id}:${stopIndex}`, title: "Collect now" }] : [{ id: `wa-collect:now:${task.id}:${stopIndex}`, title: "Collect now" }];
  ...
  if (action.startsWith("wa-mop:")) {
    ...
    collectionConfirmations.delete(from); staffProofs.delete(from); paymentProofPending.delete(from); cashCollectionPending.delete(from);
    if (mode === "cash") {
      cashCollectionPending.set(from, { taskId, stopIndex: Number(indexText), kind: kind === "partial" ? "partial" : "full", step: 0, counts: [] });
      await sendText(from, `Cash collection due: Rs.${collectionRemaining(stop).toFixed(2)}. ₹500 ke notes kitne hain? Sirf number bhejein (0 bhi chalega).`, "Collection", taskId);
    }
    else {
      ...
      if (mode === "upi") await sendCollectionQr(from, collectionRemaining(stop), taskId);
      await sendText(from, mode === "upi" ? "UPI received amount sirf number mein bhejein. Proof photo optional hai; photo bhi bhej sakte hain. Amount check karke confirmation button dabayein." : `${proofMode} proof photo bhejein. Photo ke baad confirmation button aa jayega.`, "Collection", taskId);
    }
    return true;
  }
  if (action.startsWith("wa-cash:confirm:")) {
    ...
    const party = (await getCollectionRetailer(stop.supplierId)) as { allowPartialCollection?: boolean; collectionTolerance?: number } | undefined;
    const tolerance = WHATSAPP_COLLECTION_TOLERANCE;
    if (amount > collectionRemaining(stop) + tolerance) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), amount, "Cash collection above bill amount"); await sendText(from, `Cash total Rs.${amount.toFixed(2)} bill Rs.${collectionRemaining(stop).toFixed(2)} se zyada hai. WhatsApp Admin ko alert bhej diya gaya hai; Contact Admin.`); return true; }
    const exceptionApproved = await collectionExceptionApproved(task.id);
    if (kind === "full" && amount + tolerance < collectionRemaining(stop) && !exceptionApproved) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), amount, "Full cash confirmation below bill amount"); await sendText(from, "Full collection select kiya tha, lekin cash bill se kam hai. Partial flow select karein ya WhatsApp Admin se approval lein."); return true; }
    if (kind === "partial" && amount + tolerance < collectionRemaining(stop) && !party?.allowPartialCollection && !exceptionApproved) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), amount, "Cash partial collection without privilege"); await sendText(from, "Is retailer ke liye partial collection allowed nahi hai. WhatsApp Admin ko alert bhej diya gaya hai; Contact Admin."); return true; }
    ...
    const stops: DeliveryRouteStop[] = task.routeStops.map((item, index) => index === stopIndex ? { ...item, paid: amount + tolerance >= collectionRemaining(item), collectionStatus: amount + tolerance >= collectionRemaining(item) ? "Collected" : "Pending", collectionMode: "Cash", collectionAmount: (item.collectionAmount || 0) + amount } : item);
    await updateDeliveryTask(task.id, { linkedOrderIds: task.linkedOrderIds, consignmentId: task.consignmentId, assignedTo: task.assignedTo, transportType: task.transportType, vehicleNumber: task.vehicleNumber, freightAmount: task.freightAmount, routeStops: stops, pickupAt: task.pickupAt, dropAt: task.dropAt, routeHint: task.routeHint, paymentAction: task.paymentAction, cashCollectionRequired: task.cashCollectionRequired, cashHandoverMarked: task.cashHandoverMarked, weightProofName: task.weightProofName, cashProofName: task.cashProofName, status: task.status });
  ...
  if (action.startsWith("wa-proof:confirm:")) {
    ...
    const tolerance = WHATSAPP_COLLECTION_TOLERANCE;
    const exceptionApproved = await collectionExceptionApproved(task.id);
    if (amount > collectionRemaining(stop) + tolerance) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), amount, `${mode} collection above bill amount`); await sendText(from, `${mode} amount Rs.${amount.toFixed(2)} bill Rs.${collectionRemaining(stop).toFixed(2)} se zyada hai. WhatsApp Admin ko alert bhej diya gaya hai; Contact Admin.`); return true; }
    if (kind === "full" && amount + tolerance < collectionRemaining(stop) && !exceptionApproved) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), amount, `Full ${mode} confirmation below bill amount`); await sendText(from, "Full collection select kiya tha, lekin amount bill se kam hai. Partial flow select karein ya WhatsApp Admin se approval lein."); return true; }
    if (kind === "partial" && !party?.allowPartialCollection && !exceptionApproved) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), amount, "Partial UPI/cheque without privilege"); await sendText(from, "Is retailer ke liye partial collection allowed nahi hai. WhatsApp Admin ko alert bhej diya gaya hai."); return true; }
    if (mode === "Cheque" && !(await getCollectionRetailer(stop.supplierId))?.allowChequeCollection) { await sendText(from, "Cheque privilege ab allowed nahi hai. Payment mode dobara select karein."); return true; }
    const proof = staffProofs.get(from); if (mode === "Cheque" && !proof) { await sendText(from, "Proof photo missing hai. Payment mode dobara select karein."); return true; }
    ...
    const stops: DeliveryRouteStop[] = task.routeStops.map((item, index) => index === stopIndex ? { ...item, paid: amount + tolerance >= collectionRemaining(item), collectionStatus: amount + tolerance >= collectionRemaining(item) ? "Collected" : "Pending", collectionMode: mode, collectionAmount: (item.collectionAmount || 0) + amount, collectionProofName: proof } : item);
    await updateDeliveryTask(task.id, { linkedOrderIds: task.linkedOrderIds, consignmentId: task.consignmentId, assignedTo: task.assignedTo, transportType: task.transportType, vehicleNumber: task.vehicleNumber, freightAmount: task.freightAmount, routeStops: stops, pickupAt: task.pickupAt, dropAt: task.dropAt, routeHint: task.routeHint, paymentAction: task.paymentAction, cashCollectionRequired: task.cashCollectionRequired, cashHandoverMarked: task.cashHandoverMarked, weightProofName: task.weightProofName, cashProofName: task.cashProofName, status: task.status });
  ...
  if (action === "wa-settlement:confirm") {
    if (!staffHasRole(user, ["Admin", "Delivery", "Out Delivery", "Collection Agent"])) { await sendText(from, "Collection access required hai."); return true; }
    ...
    const totals = await executeDatabaseQuery<Record<string, unknown>>(`SELECT COALESCE(SUM(amount),0) AS amount FROM payments WHERE side='Sales' AND created_by=$1 AND reference_number LIKE 'WA-%' AND created_at > COALESCE($2::timestamptz,'epoch'::timestamptz)`, [user.fullName, since.rows[0]?.created_at || null]);
    if (numberValue(totals.rows[0]?.amount) <= 0) { await sendText(from, "Settlement ke liye nayi collection nahi hai."); return true; }
  ...
  if (deliveryUser && cashSession) {
    ...
    if (!Number.isFinite(count) || count < 0 || count > 10000 || (cashSession.step < cashDenominations.length ? !Number.isInteger(count) : Math.abs(count * 100 - Math.round(count * 100)) > 0.000001)) { await sendText(from, `Sirf valid number bhejein. ₹${cashSession.step < cashDenominations.length ? cashDenominations[cashSession.step] : "coins"} ke liye quantity chahiye.`); return true; }
    ...
    const party = (await getCollectionRetailer(stop.supplierId)) as { allowPartialCollection?: boolean; collectionTolerance?: number } | undefined;
    const tolerance = WHATSAPP_COLLECTION_TOLERANCE; const short = collectionRemaining(stop) - total;
    if (short < -tolerance || (short > tolerance && !party?.allowPartialCollection)) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), total, short < -tolerance ? "Cash collection above bill amount" : "Cash short collection without partial privilege"); await sendText(from, `Cash total Rs.${total.toFixed(2)}, bill Rs.${collectionRemaining(stop).toFixed(2)}. WhatsApp Admin ko alert bhej diya gaya hai; Contact Admin.`); return true; }
    collectionConfirmations.set(from, `wa-cash:confirm:${cashSession.kind}:${task.id}:${cashSession.stopIndex}:${total}`);
    await sendButtons(from, `Cash total: Rs.${total.toFixed(2)}\nBill: Rs.${collectionRemaining(stop).toFixed(2)}${short > tolerance ? `\nBalance: Rs.${short.toFixed(2)}` : "\nTally OK"}\n\nConfirm karke collection record karein.`, [{ id: `wa-cash:confirm:${cashSession.kind}:${task.id}:${cashSession.stopIndex}:${total}`, title: "Confirm cash" }], "Collection", task.id);
    return true;
  }
  if (deliveryUser && paymentProofPending.has(from) && (normalized.startsWith("AMOUNT ") || /^\d+(?:\.\d{1,2})?$/.test(command))) {
    const session = paymentProofPending.get(from)!;
    if (session.mode !== "UPI") { await sendText(from, "Amount payment photo se read hoga. Clear cheque photo bhejein."); return true; }
    const amountText = command.replace(/^AMOUNT\s+/i, "");
    const amount = Number(amountText);
    if (!/^\d+(?:\.\d{1,2})?$/.test(amountText) || !Number.isFinite(amount) || amount <= 0) { await sendText(from, "Valid received amount sirf number mein bhejein (jaise 100.50)."); return true; }
    const task = snapshot.deliveryTasks.find((item) => item.id === session.taskId);
    const stop = task?.routeStops[session.stopIndex];
    if (!task || !deliveryTaskAllowed(task, user) || !stop || stop.paid || !stop.delivered) { await sendText(from, "Collection unavailable hai. LIST se retailer dobara select karein."); return true; }
    const confirmation = `wa-proof:confirm:upi:${session.kind}:${task.id}:${session.stopIndex}:${amount}`;
    collectionConfirmations.set(from, confirmation);
    await sendButtons(from, `UPI received: Rs.${amount.toFixed(2)}\nBill: Rs.${collectionRemaining(stop).toFixed(2)}\nProof photo optional hai. Amount check karke confirm karein.`, [{ id: confirmation, title: "Confirm UPI" }], "Collection", task.id);
    return true;
  }
  ...
  if (deliveryUser && (normalized === "TOTAL" || normalized === "SUM" || normalized === "LIST COLLECTION" || normalized === "SETTLE")) {
    const since = await executeDatabaseQuery<{ created_at: string }>(`SELECT created_at FROM note_records WHERE entity_type='Delivery' AND entity_id=$1 AND note LIKE 'WhatsApp settlement%' ORDER BY created_at DESC LIMIT 1`, [String(user.id)]);
    ...
    if (normalized === "LIST COLLECTION") {
      ...
      if (normalized === "TOTAL" || normalized === "SUM") await notifyDeliveryAllDone(from, user);
    }
    return true;
  }
  ...
  if (deliveryUser && normalized.startsWith("CASH ")) {
    ...
    const party = (await getCollectionRetailer(stop.supplierId)) as { allowPartialCollection?: boolean; collectionTolerance?: number } | undefined;
    const tolerance = WHATSAPP_COLLECTION_TOLERANCE; const short = collectionRemaining(stop) - total;
    if (short < -tolerance || (short > tolerance && !party?.allowPartialCollection)) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), total, short < -tolerance ? "Cash collection above bill amount" : "Cash short collection without partial privilege"); await sendText(from, `Cash total Rs.${total.toFixed(2)} hai, bill Rs.${collectionRemaining(stop).toFixed(2)} hai. Difference Rs.${short.toFixed(2)}. WhatsApp Admin ko alert bhej diya gaya hai; Contact Admin.`); return true; }
    await sendText(from, `Cash counted: Rs.${total.toFixed(2)}. Bill: Rs.${collectionRemaining(stop).toFixed(2)}.${short > tolerance ? ` Pending: Rs.${short.toFixed(2)}.` : " Tally OK."}\nAb proof photo bhejkar COLLECT ${shortId(task.id)} ${stopIndex + 1} ${total.toFixed(2)} CASH type karein.`, "CashCount", task.id);
    return true;
  }
  ...
  if (deliveryUser && normalized.startsWith("COLLECT ")) {
    ...
    const tolerance = WHATSAPP_COLLECTION_TOLERANCE; if (mode !== "LATER" && amount + tolerance < collectionRemaining(stop) && !privilege?.allowPartialCollection) { await alertWhatsAppAdminForCollection(task.id, stop.supplierName, collectionRemaining(stop), amount, "Short collection without partial privilege"); await sendText(from, `Full collection required: ₹${collectionRemaining(stop).toFixed(2)}. WhatsApp Admin ko approval alert bhej diya gaya hai; Contact Admin.`); return true; }
    const proof = staffProofs.get(from);
    if (mode !== "LATER" && mode !== "UPI" && !proof) { await sendText(from, "Pehle cash/cheque proof photo bhejein, phir COLLECT command type karein."); return true; }
...
async function handleInboundMessage(message: JsonObject) {
  ...
  if ((await executeDatabaseQuery("SELECT id FROM whatsapp_shortage_cases WHERE draft_id=$1 OR id IN (SELECT case_id FROM whatsapp_shortage_portions WHERE draft_id=$1)", [draftId])).rowCount) throw new Error("Manage this order from the shortage register so remaining quantities stay tracked.");
      ...
      const changed=await executeDatabaseQuery(`UPDATE whatsapp_order_drafts SET status = 'Change Requested', confirmation_message_id=NULL WHERE id = $1 AND counterparty_id = $2 AND status IN ('Needs Review','Change Requested','Staff Approved','Awaiting Retailer') RETURNING id`, [draftId, profile.counterpartyId]);
      if(!changed.rowCount)throw new Error("This confirmation is no longer editable. Contact Sales for a new order.");
  ...
  if ((await executeDatabaseQuery("SELECT id FROM whatsapp_shortage_cases WHERE draft_id=$1 OR id IN (SELECT case_id FROM whatsapp_shortage_portions WHERE draft_id=$1)", [draftId])).rowCount) throw new Error("Manage this order from the shortage register so remaining quantities stay tracked.");
      ...
      if(text(loaded.draft.status)!=="Change Requested")throw new Error("This confirmation is no longer editable. Contact Sales for a new order.");
  ...
  if ((await executeDatabaseQuery("SELECT id FROM whatsapp_shortage_cases WHERE draft_id=$1 OR id IN (SELECT case_id FROM whatsapp_shortage_portions WHERE draft_id=$1)", [draftId])).rowCount) throw new Error("Manage this order from the shortage register so remaining quantities stay tracked.");
    ...
    if (body) {
      ...
      if (draftChangeMatch && changeSession?.selectedProductSku) {
        ...
        if ((await executeDatabaseQuery("SELECT id FROM whatsapp_shortage_cases WHERE draft_id=$1 OR id IN (SELECT case_id FROM whatsapp_shortage_portions WHERE draft_id=$1)", [draftId])).rowCount) throw new Error("Contact Sales to revise an order with tracked pending quantities.");
        const loaded = await loadDraft(draftId);
...
export async function removeWhatsAppStaffUser(userId: number, currentUser: StaffUser) {
  if (!isWhatsAppAdminUser(currentUser)) throw new Error("Only the WhatsApp admin can remove users.");
  if (String(userId) === String(currentUser.id)) throw new Error("You cannot remove your own account.");
  const found = await executeDatabaseQuery<StaffUser & { mobileNumber: string }>(
    `SELECT id,username,full_name AS "fullName",role,roles_json AS roles,mobile_number AS "mobileNumber" FROM users WHERE id=$1 AND active=TRUE`, [userId]
  );
  const target = found.rows[0];
  if (!target) throw new Error("Active user not found.");
  if (target.role === "Admin" || isWhatsAppAdminUser(target)) throw new Error("Admin accounts cannot be removed from WhatsApp Admin.");
  // Preserve the user ID/username on historical orders and payments, but revoke
  // authentication and free the phone number for a new registration atomically.
  await executeDatabaseQuery(
    `WITH removed AS (
       UPDATE users SET active=FALSE,mobile_number='',warehouse_ids_json='[]'::jsonb
       WHERE id=$1 RETURNING id
     ), revoked AS (
       DELETE FROM sessions WHERE user_id IN (SELECT id FROM removed)
     )
     DELETE FROM settings WHERE key IN ('whatsapp_test_mode:' || $1::text,'whatsapp_test_delivery:' || $1::text)
       OR (key LIKE 'dco-checkbox-%' AND value_json->>'userId'=$1::text)`, [userId]
  );
  let phone = "";
  try { phone = normalizeWhatsAppPhone(target.mobileNumber); } catch { /* A missing/invalid old number must not prevent access removal. */ }
  for (const state of [staffProofs, deliveryProofPending, cashCollectionPending, paymentProofPending,
    packingPhotoPending, packingWeightResults, packingPhotoProofs, packingManualWeightPending,
    packingChangePending, dcoBuildSessions, dcoHandoverSelections, receiptSessions, collectionConfirmations]) state.delete(phone);
  return getSnapshot();
}
  ...
  try {
    const legacy = await executeDatabaseQuery<{ id: string }>(`SELECT d.id FROM whatsapp_order_drafts d
      WHERE d.status IN ('Awaiting Retailer','Needs Review','Change Requested')
        ...
        AND NOT EXISTS(SELECT 1 FROM whatsapp_shortage_cases s WHERE s.draft_id=d.id OR s.id IN (SELECT case_id FROM whatsapp_shortage_portions WHERE draft_id=d.id))
...
async function sendShortageNotification(notification: Record<string, unknown>) {
  ...
  const pending=lines.filter(l=>numberValue(l.pending_quantity)-numberValue(l.released_quantity)>0).map(l=>`${text(l.name)}: ${numberValue(l.pending_quantity)-numberValue(l.released_quantity)}`).join("\n");
  ...
  if(notification.kind==='SupplyAlert') {
    const recipients=await executeDatabaseQuery<Record<string,unknown>>(`SELECT id,mobile_number FROM users WHERE active=TRUE AND
      (id=$1 OR id=$2 OR role='Admin' OR roles_json ? 'Admin' OR lower(username)=ANY($3::text[])) ORDER BY id`,[row.salesman_id,row.purchaser_id,[...whatsappAdminUsernames()]]);
    if(!recipients.rowCount)throw new Error('No staff recipients are available for supplier follow-up.');
    const items=Array.isArray(payload.items)?payload.items as Record<string,unknown>[]:[];
    const detail=items.map(item=>`${item.product_sku}: ordered ${item.quantity_ordered}, received ${item.quantity_received}, outstanding ${Math.max(0,numberValue(item.quantity_ordered)-numberValue(item.quantity_received))}`).join('\n');
  ...
  if(notification.kind==='SupplyDecision') {
    const update=payload.decision==='Wait'?`The revised expected arrival is ${new Date(String(payload.expectedAt)).toLocaleString('en-IN',{timeZone:'Asia/Kolkata'})} IST. The remaining quantity stays pending until received.`:payload.decision==='Dispatch'?'Sales has recorded your request to dispatch available stock. Available quantities will receive a separate confirmation; the remaining quantity stays pending.':'The remaining quantity without a prepared confirmation has been cancelled as agreed. Previously confirmed or prepared portions remain active.';
    await sendText(phone,`Order ${row.draft_id}\n${update}`,'Shortage',text(row.id));return;
  }
...
export async function processWhatsAppConfirmations() {
  if(confirmationSweepRunning)return;
  confirmationSweepRunning=true;
  try {
    await confirmationService.reconcile();
    for(let index=0;index<30;index++) {
      const claimed=await executeDatabaseQuery<Record<string,unknown>>(`UPDATE whatsapp_confirmation_notifications SET status='Sending',attempts=attempts+1,available_at=NOW()+INTERVAL '5 minutes'
        WHERE id=(SELECT id FROM whatsapp_confirmation_notifications WHERE status IN ('Pending','Sending') AND available_at<=NOW() ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 1) RETURNING *`);
      const notification=claimed.rows[0];if(!notification)break;
      try {
        const sent=await sendConfirmationFollowupNotification(notification);
        await executeDatabaseQuery("UPDATE whatsapp_confirmation_notifications SET status=$2,last_error='' WHERE id=$1",[notification.id,sent?'Sent':'Superseded']);
      } catch(error) {
        await executeDatabaseQuery("UPDATE whatsapp_confirmation_notifications SET status=$2,last_error=$3,available_at=NOW()+INTERVAL '5 minutes' WHERE id=$1",[notification.id,numberValue(notification.attempts)>=5?'Failed':'Pending',error instanceof Error?error.message:'Notification failed']);
      }
    }
  } finally {confirmationSweepRunning=false;}
}

async function sendConfirmationFollowupNotification(notification:Record<string,unknown>) {
  const row=(await executeDatabaseQuery<Record<string,unknown>>(`SELECT d.*,f.version,f.due_at,c.name AS retailer_name,u.full_name AS salesman_name
    FROM whatsapp_order_drafts d JOIN whatsapp_confirmation_followups f ON f.draft_id=d.id JOIN counterparties c ON c.id=d.counterparty_id LEFT JOIN users u ON u.id=d.salesman_id WHERE d.id=$1`,[notification.draft_id])).rows[0];
  if(!row)return false;
  const payload=(notification.payload_json||{}) as Record<string,unknown>;
  if(notification.kind==='Cancelled') {
    if(row.status!=='Denied')return false;
    await sendText(text(row.phone_e164),`Order confirmation ${row.id} has been cancelled. Reason: ${payload.reason}.${payload.shortageCaseId?' This cancellation applies only to this unconfirmed portion. Other confirmed orders and pending quantities remain active with Sales.':''}`,'ConfirmationCancellation',text(notification.id));
    return true;
  }
  if(row.status!=='Awaiting Retailer'||numberValue(row.version)!==numberValue(notification.version))return false;
  if(notification.kind==='Resend') {
    const loaded=await loadDraft(text(row.id));
    const lines=loaded.lines.filter(line=>numberValue(line.approved_quantity)>0);
    if(!lines.length)throw new Error('There are no available items to confirm. Sales must resolve the order.');
    const sent=await sendButtons(text(row.phone_e164),compactProforma(text(row.id),loaded.draft,lines),[
      {id:`wa-confirm:${row.id}`,title:'Confirm Order'},{id:`wa-change:${row.id}`,title:'Request Change'},{id:`wa-proforma:${row.id}`,title:'View Proforma'}
    ],'ConfirmationReminder',text(notification.id));
    await executeDatabaseQuery("UPDATE whatsapp_order_drafts SET confirmation_message_id=$2 WHERE id=$1 AND status='Awaiting Retailer'",[row.id,sent.messageId]);
    return true;
  }
  if(notification.kind==='Overdue') {
    if(!row.due_at||Date.parse(String(row.due_at))>Date.now())return false;
    const recipients=(await executeDatabaseQuery<Record<string,unknown>>(`SELECT id,mobile_number FROM users WHERE active=TRUE AND
      (id=$1 OR role='Admin' OR roles_json ? 'Admin' OR lower(username)=ANY($2::text[])) ORDER BY id`,[row.salesman_id,[...whatsappAdminUsernames()]])).rows;
    if(!recipients.length)throw new Error('No Sales or WhatsApp Admin recipient is available.');
    const failures:string[]=[];
...
async function downloadReturnPhoto(mediaId:string){
  const headers={Authorization:`Bearer ${process.env.WHATSAPP_ACCESS_TOKEN}`};
  const metadata=await fetch(`${graphBase}/${encodeURIComponent(mediaId)}`,{headers,signal:AbortSignal.timeout(20000)});
  if(!metadata.ok)throw Error('Unable to retrieve the WhatsApp photo. Please send it again.');
  const info=await metadata.json() as {url?:string;file_size?:number};
  if(!info.url||Number(info.file_size)>8*1024*1024)throw Error('Send a photo no larger than 8 MB.');
  const response=await fetch(info.url,{headers,signal:AbortSignal.timeout(20000)});
  if(!response.ok||Number(response.headers.get('content-length'))>8*1024*1024)throw Error('Unable to download this photo. Send a photo no larger than 8 MB.');
  const reader=response.body?.getReader();if(!reader)throw Error('Photo download is empty.');const chunks:Uint8Array[]=[];let size=0;
  for(;;){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>8*1024*1024){await reader.cancel();throw Error('Send a photo no larger than 8 MB.');}chunks.push(value);}
  return Buffer.concat(chunks);
}
async function sendReturnEvidence(phone:string,row:any,user:StaffUser){
  for(const photo of row.photos||[]){
    const stored=await deliveryExceptionService.photo(row.id,photo.id,user,isWhatsAppAdminUser(user));
    const form=new FormData();form.append('messaging_product','whatsapp');form.append('type','image/jpeg');form.append('file',new Blob([new Uint8Array(stored.image_bytes)],{type:'image/jpeg'}),'evidence.jpg');
    const result=await fetch(`${graphBase}/${process.env.WHATSAPP_PHONE_NUMBER_ID}/media`,{method:'POST',headers:{Authorization:`Bearer ${process.env.WHATSAPP_ACCESS_TOKEN}`},body:form,signal:AbortSignal.timeout(20000)});
    const data=await result.json() as {id?:string};if(!result.ok||!data.id)throw Error('Unable to display evidence on WhatsApp. Photos remain available in BConnect.');
    const product=row.original_json.find((l:any)=>l.id===photo.lineId)?.product_name||'Visit / handover';
    await sendGraphMessage(phone,{type:'image',image:{id:data.id,caption:compact(`${photo.stage} | ${product}\n${photo.createdBy} | ${new Date(photo.createdAt).toLocaleString('en-IN',{timeZone:'Asia/Kolkata'})}`,1024)}},'DeliveryException',row.id);
  }
  if(!row.photos?.length)await sendText(phone,'No photo evidence attached yet.');
}
async function sendDeliveryExceptionCard(phone:string,row:any){
  ...
  const billing=row.bill_adjusted_at?`Revised bill: Rs.${Number(row.adjusted_total).toFixed(2)}\nAlready paid: Rs.${Number(row.paid_amount||0).toFixed(2)}\nCollect now: Rs.${Number(row.amount_due||0).toFixed(2)}\n`:row.proposed_bill?`Proposed bill: Rs.${Number(row.proposed_bill.total).toFixed(2)}\n`:'';
  const body=compact(`${billing}${report.kind} — ${row.retailer_name||row.order_id}\nReport: ${row.id}\nStatus: ${row.status}\n${lines||'No return products selected yet.'}\nReason: ${report.reason||'Required'}\nGoods custody: ${row.warehouse_received_at?"Warehouse received":row.agent_username}.\n${row.decision_note||''}${row.retry_at?`\nRetry: ${new Date(row.retry_at).toLocaleString('en-IN',{timeZone:'Asia/Kolkata'})}`:''}`,1024);
  const buttons=[];
  if(row.bill_adjusted_at&&row.canEdit&&Number(row.amount_due)>0)buttons.push({id:`wa-ex:collect:${row.id}`,title:'Collect payment'});
  if(['Draft','Correction Requested'].includes(row.status)&&row.canEdit!==false){buttons.push({id:`wa-ex:edit:${row.id}`,title:'Edit report'},{id:`wa-ex:photos:${row.id}:report:0`,title:'Add photo'},{id:`wa-ex:submit:${row.id}:${row.revision}`,title:'Confirm and send'});}
  else if((row.status==='Awaiting Seller'||(row.status==='Return Authorized'&&!row.bill_adjusted_at))&&row.canDecide){buttons.push({id:`wa-ex:decision:${row.id}:${row.revision}:Return`,title:'Approve revised bill'},{id:`wa-ex:decision:${row.id}:${row.revision}:Correction`,title:'Request correction'});if(report.kind==='Shop closed')buttons.push({id:`wa-ex:decision:${row.id}:${row.revision}:Retry`,title:'Retry delivery'});}
  if(row.bill_adjusted_at&&row.canEdit&&!row.warehouse_received_at)buttons.push({id:`wa-ex:photos:${row.id}:handover:0`,title:'Handover photo'},...(row.handover_at?[]:[{id:`wa-ex:handover:${row.id}:${row.revision}`,title:'Confirm handover'}]));
  if(buttons.length)await sendButtons(phone,body,buttons.slice(0,3),'DeliveryException',row.id);else await sendText(phone,body,'DeliveryException',row.id);
  if(row.canReceive&&row.handover_at&&!row.warehouse_received_at){
    const counts=row.receipt_json?row.receipt_json.lines.map((l:any)=>`${row.original_json.find((o:any)=>o.id===l.id)?.product_name}: sellable ${l.good}, damaged ${l.damaged}, missing ${row.report_json.lines.find((r:any)=>r.id===l.id).quantity-l.good-l.damaged}`).join('\n')+'\nFindings: '+row.receipt_json.note:'Record the physical count, condition and photo evidence before confirming receipt.';
    await sendButtons(phone,compact(counts,1024),[{id:`wa-ex:count:${row.id}:${row.revision}`,title:'Record / edit count'},{id:`wa-ex:photos:${row.id}:receipt:0`,title:'Receiving photos'},...(row.receipt_json?[{id:`wa-ex:receive:${row.id}:${row.revision}`,title:'Confirm receipt'}]:[])],'DeliveryException',row.id);
  }
  if(row.photos?.length)await sendButtons(phone,`${row.photos.length} evidence photo(s) attached.`,[{id:`wa-ex:evidence:${row.id}`,title:'View photo evidence'}],'DeliveryException',row.id);
}
async function handleDeliveryExceptionMessage(message:JsonObject,from:string,user:StaffUser){
  ...
  if(action.startsWith('wa-ex:')){
    ...
    if(kind!=='decide-confirm')await executeDatabaseQuery('DELETE FROM settings WHERE key=$1',[sessionKey]);
    ...
    if(kind==='evidence'){await sendReturnEvidence(from,row,user);return true;}
    if(kind==='photos'){
      const stage=part==='report'?'Agent report':part==='handover'?'Agent handover':part==='receipt'?'Warehouse receipt':'';
      if(!stage)throw Error('Unknown evidence stage.');
      if(part==='handover'||(part==='report'&&row.report_json.kind==='Shop closed')){await deliveryExceptionService.authorizePhoto(id,stage,null,user,admin);await setInput({id,stage:'photo',photoStage:stage,lineId:null});await sendText(from,`Take and send a photo now: ${stage}. Send CANCEL to stop.`);return true;}
      const page=Math.max(0,Number(extra)||0);const selected=row.report_json.lines;if(!selected.length)throw Error('Save returned products and quantities before attaching photos.');
      const rows=selected.slice(page*9,page*9+9).map((l:any)=>({id:`wa-ex:photo:${id}:${part}:${l.id}`,title:compact(row.original_json.find((o:any)=>o.id===l.id)?.product_name||l.id,24),description:`${stage} | Return ${l.quantity}`}));
      if(selected.length>(page+1)*9)rows.push({id:`wa-ex:photos:${id}:${part}:${page+1}`,title:'More products',description:'Next products'});
      await sendGraphMessage(from,{type:'interactive',interactive:{type:'list',body:{text:`Select a product for ${stage.toLowerCase()} evidence. Photograph any damage clearly.`},action:{button:'Select product',sections:[{title:'Photo evidence',rows}]}}},'DeliveryException',id);return true;
    }
    if(kind==='photo'){const stage=part==='report'?'Agent report':part==='receipt'?'Warehouse receipt':'';await deliveryExceptionService.authorizePhoto(id,stage,extra,user,admin);await setInput({id,stage:'photo',photoStage:stage,lineId:extra});await sendText(from,`Take and send a clear photo of this product and its condition now. Stage: ${stage}. Send CANCEL to stop.`);return true;}
    if(kind==='handover'){await deliveryExceptionService.handover(id,Number(part),user,admin);await sendDeliveryExceptionCard(from,await deliveryExceptionService.get(id,user,admin));void processDeliveryExceptions();return true;}
    if(kind==='count'){if(!row.canReceive||!row.handover_at||row.warehouse_received_at)throw Error('Only the assigned Warehouse Manager can count a return awaiting receipt.');await setInput({id,revision:Number(part),stage:'receipt-count'});await sendText(from,compact(`Enter sellable and damaged counts for EVERY numbered product, then your findings.\n${row.report_json.lines.map((l:any,i:number)=>`${i+1}. ${row.original_json.find((o:any)=>o.id===l.id)?.product_name}: expected ${l.quantity}`).join('\n')}\nFormat: 1=2,1;2=3,0 | Findings\nIf units are missing, explain the final decision and append | RESOLVE MISSING. You will review before confirming.`,4096));return true;}
    if(kind==='receive'){await deliveryExceptionService.finalizeReceipt(id,Number(part),user);await sendDeliveryExceptionCard(from,await deliveryExceptionService.get(id,user,admin));void processDeliveryExceptions();return true;}

    if(kind==='collect'){
      if(!row.canEdit||!row.bill_adjusted_at||Number(row.amount_due)<=0)throw Error('No approved outstanding collection is available for this agent.');
      const snapshot=await getSnapshot(user);const task=snapshot.deliveryTasks.find(t=>t.id===row.task_id);const index=task?.routeStops.findIndex(s=>s.orderId===row.order_id)??-1;
      if(!task||index<0||!deliveryTaskAllowed(task,user))throw Error('Assigned delivery stop unavailable.');
      return handleStaffWhatsAppMessage({interactive:{button_reply:{id:`wa-collect:now:${task.id}:${index}`}}},from,user);
    }
    if(kind==='products'){
    ...
    if(kind==='outcome'){if(!row.canEdit)throw Error('Only the delivery agent can edit the report.');await sendButtons(from,'Choose the report type. The report remains a draft until Confirm and send.',[{id:`wa-ex:type:${id}:closed`,title:'Shop closed'},{id:`wa-ex:type:${id}:returned`,title:'Returned'},{id:`wa-ex:withdraw:${id}:${row.revision}`,title:'Discard draft'}],'DeliveryException',id);return true;}
    if(kind==='edit'){if(!row.canEdit)throw Error('Only the assigned agent can edit.');await sendButtons(from,'Select what to edit.',[{id:`wa-ex:products:${id}:0`,title:'Products / quantity'},{id:`wa-ex:outcome:${id}`,title:'Outcome / reason'},{id:`wa-ex:withdraw:${id}:${row.revision}`,title:'Discard draft'}],'DeliveryException',id);return true;}
  ...
  if(body.toUpperCase()==='EXCEPTIONS'){await executeDatabaseQuery('DELETE FROM settings WHERE key=$1',[sessionKey]);const {cases}=await deliveryExceptionService.list(user,admin);const rows=cases.slice(0,10).map((r:any)=>({id:`wa-ex:view:${r.id}`,title:compact(r.retailer_name,24),description:compact(`${r.status} | ${r.report_json.kind}`,72)}));if(!rows.length)await sendText(from,'No delivery exception reports.');else await sendGraphMessage(from,{type:'interactive',interactive:{type:'list',body:{text:'Select a delivery exception. All reports are also available in BConnect.'},action:{button:'Reports',sections:[{title:'Delivery exceptions',rows}]}}},'DeliveryException');return true;}
  ...
  if(pending.stage==='photo'){
    const mediaId=text((message.image as JsonObject|undefined)?.id);if(!mediaId){await sendText(from,'Send a photo as an image, or send CANCEL to stop.');return true;}
    await deliveryExceptionService.authorizePhoto(row.id,pending.photoStage,pending.lineId,user,admin);
    await deliveryExceptionService.addPhoto(row.id,pending.photoStage,pending.lineId,await downloadReturnPhoto(mediaId),user,admin,'WhatsApp',text(message.id)||undefined);
    await executeDatabaseQuery('DELETE FROM settings WHERE key=$1',[sessionKey]);await sendText(from,'Photo evidence saved.');await sendDeliveryExceptionCard(from,await deliveryExceptionService.get(row.id,user,admin));return true;
  }
  if(!body)return false;
  if(pending.stage==='receipt-count'){
    const [raw,note,resolve]=body.split('|');const entries=raw.split(';').map(v=>v.trim());
    const used=new Set<number>();const lines=entries.map(entry=>{const match=entry.match(/^(\d+)\s*=\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)$/);if(!match)throw Error('Use product number=sellable,damaged; then | findings.');const index=Number(match[1])-1;const line=row.report_json.lines[index];if(!line||used.has(index))throw Error('Include every product number exactly once.');used.add(index);return {id:line.id,good:Number(match[2]),damaged:Number(match[3])};});
    await deliveryExceptionService.saveReceipt(row.id,{lines,note:note?.trim()||'',resolveMissing:resolve?.trim().toUpperCase()==='RESOLVE MISSING'},pending.revision,user);
    await executeDatabaseQuery('DELETE FROM settings WHERE key=$1',[sessionKey]);await sendDeliveryExceptionCard(from,await deliveryExceptionService.get(row.id,user,admin));return true;
  }
