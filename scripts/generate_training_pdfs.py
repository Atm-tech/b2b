...
make_pdf("04_Warehouse_IN_OUT_Guide.pdf", "WAREHOUSE - WHATSAPP IN / OUT", "For: Warehouse Manager", "WhatsApp se stock inward, sales dispatch aur DCO banana. Har action ka actual record B CONNECT mein banta hai.", "Apne registered WhatsApp se command type karo: IN, OUT, READY aur DCO.", [
    ...
    ("OUT - sales dispatch and DCO", "Confirmed sales orders ko warehouse WhatsApp se ready karo, phir same warehouse ke multiple bills ek DCO mein bundle karo.", ["OUT type karke ready sales orders dekho.", "Last digits se correct order choose karo: OUT 482516.", "Packed weight/photo bhejo.", "Type karo: READY 482516. Isse docket ready hota hai.", "2-4 ready orders ek route mein bhejne ho to type karo: DCO 482516,639104,774210.", "System ek DCO banakar registered Delivery + Collection agent ko task de deta hai.", "Reply mein DCO ID aur agent name check karo."], ("stop", "Do not dispatch", "Wrong retailer, missing weight photo, wrong quantity, ya different warehouse orders ko ek DCO mein mat bhejo.")),
...
make_pdf("05_Delivery_Collection_Guide.pdf", "DELIVERY + COLLECTION - WHATSAPP", "For: One Delivery and Collection Agent", "WhatsApp par assigned DCO dekhna, proof ke saath delivery karna aur permitted collection record karna.", "Aap ek hi agent ho. Command READ se shuru karo; delivery ke baad wahi collection command use hogi.", [
    ("Start delivery from WhatsApp", "Sirf apne registered WhatsApp number se kaam karo.", ["WhatsApp chat mein READ type karo.", "Reply mein task last 6 digits aur retailer stop number dekho.", "Retailer par shop name aur order verify karo.", "Parcel ke saath clear delivery photo bhejo.", "Type karo: DELIVERED 482516 1. Ismein task last 6 digits aur stop number hota hai.", "System collection instruction reply karega."], ("warn", "Proof is compulsory", "Photo ke bina DELIVERED final nahi hoga. Photo mein parcel aur shop context clear hona chahiye.")),
    ("Collection from WhatsApp", "Delivery ke baad retailer privilege ke hisaab se collection record karo.", ["UPI/cash/cheque proof photo pehle WhatsApp par bhejo.", "Type karo: COLLECT 482516 1 640.00 UPI.", "Cash ke liye CASH, cheque ke liye CHEQUE type karo.", "Later collection sirf allowed retailer ke liye: COLLECT 482516 1 0 LATER.", "Partial amount sirf privilege wale retailer par accept hoga.", "System reply mein recorded amount aur pending work check karo.", "Next task ke liye READ type karo."], ("stop", "Money safety", "Short cash, unauthorised cheque ya proof missing ho to collection record mat karo; WhatsApp Admin ko bolo.")),
    ("After all stops", "Aaj ki duty proper close karo.", ["Pending retailer/delivery dekho.", "Collect Later cases ko pending hi rehne do; fake complete mat karo.", "All completed ho to Delivery all done status check karo.", "Cash handover instruction mile to accounts/warehouse process follow karo."], None),
])
