...
make_pdf("04_Warehouse_IN_OUT_Guide.pdf", "WAREHOUSE - WHATSAPP IN / OUT", "For: Warehouse Manager", "WhatsApp se stock inward, sales dispatch aur DCO banana. Har action ka actual record B CONNECT mein banta hai.", "Apne registered WhatsApp se command type karo: IN, SO, DCO aur HANDOVER.", [
    ("IN - purchase stock receive", "Purchase ka maal aaya? Warehouse WhatsApp par PO aur product select karke actual receipt record karo.", ["Apne registered WhatsApp se IN type karo.", "Active PO list mein supplier PO select karo.", "Product select karo. Ordered, already received aur pending quantity screen par dikhegi.", "Weighing scale ke saath clear weight photo bhejo.", "System received quantity poochega. Actual quantity type karo; partial ho to sirf jitna aaya utna likho.", "Phir gross weight kg type karo, example 8.45.", "System receipt aur stock record karega. Receive another se same PO ka agla product lo, ya Finish inward dabao."], ("warn", "Photo and actual quantity", "Photo ke bina receipt nahi karna. Supplier invoice quantity ko actual received quantity mat samjho.")),
    ("SO packing, DCO and handover", "Warehouse outbound WhatsApp flow: SO select karo, weight photo verify karo, phir packed SO ko DCO mein bundle karke agent ko handover karo.", ["SO type karo. List mein dispatch-ready sales order select karo.", "Order ki product list dekho. Packed weight photo bhejo.", "Token-free local OCR scale reading try karega aur expected weight/tolerance se compare karega. Reading clear ho to result check karo; unclear ho to actual kg manually type karo.", "Weight match ho to Packed dabao. Mismatch ho to Change dabao, product select karo aur new quantity type karo. Qty 0 se product remove hoga.", "DCO type karo. Packed SO ek-ek karke select karo, Add another se multiple SO jodo, phir Create DCO dabao.", "Delivery + Collection agent select karo. DCO ready rahega, lekin agent ko abhi visible nahi hoga.", "Physical bundle agent ko dene ke baad type karo: HANDOVER 482516. Multiple DCO ke liye comma use karo.", "Reply mein handover success check karo; tab agent ko WhatsApp message aur LIST task milega."], ("stop", "Do not dispatch", "Weight mismatch, wrong retailer, wrong quantity, ya different warehouse orders ko ek DCO mein mat bhejo.")),
...
make_pdf("05_Delivery_Collection_Guide.pdf", "DELIVERY + COLLECTION - WHATSAPP", "For: One Delivery and Collection Agent", "WhatsApp par assigned DCO dekhna, proof ke saath delivery karna aur permitted collection record karna.", "Aap ek hi agent ho. LIST se shuru karo; delivery ke baad buttons follow karo.", [
    ...
    ("Collection from WhatsApp", "Buttons sirf retailer ke approved privileges ke hisaab se dikhte hain.", ["Allowed ho to Collect now ya Collect later choose karo.", "Collect now par Full/Partial sirf approved retailer ke liye aayega.", "Cash/UPI/Cheque options mein cheque sirf privileged retailer ko milega.", "Cash: system Rs.500, Rs.200, Rs.100, Rs.50, Rs.20, Rs.10 aur coins ek-ek karke poochega. Sirf quantity likho, total check karo aur Confirm cash dabao.", "UPI: QR aayega; payment proof photo bhejo. Token-free local OCR amount read karega; clear reading ho to confirmation button check karke dabao. Na read ho to received amount sirf number mein type karo.", "Cheque: cheque photo bhejo. OCR payee aur amount clear padh paaye to confirmation aayega; warna amount manually type karo aur Accounts/Admin verification ka wait karo.", "System amount aur tolerance check karke collection record karega."], ("stop", "Money safety", "Short cash, unauthorised cheque ya proof missing ho to collection record mat karo; Contact WhatsApp Admin.")),
...
make_pdf("07_WhatsApp_Admin_Chat_Guide.pdf", "WHATSAPP ADMIN - CHAT GOVERNANCE", "For: WhatsApp Admin", "Retailer onboarding, mapping, live chat routing and service governance ek control room se chalana.", "WhatsApp message channel hai; final record B CONNECT ke andar hota hai.", [
    ...
    ("Team setup", "Operational team ka mobile/role yahin register hota hai.", ["Team tab kholo.", "Name, username, WhatsApp number, role and warehouse bharo.", "Delivery + Collection ek hi agent ho to uske same user par delivery role aur Collection Agent dono assign karo.", "Directory mein entry aur WhatsApp number verify karo.", "Sales/Purchase/Warehouse/Delivery/Collection roles correct rakho."], None),
