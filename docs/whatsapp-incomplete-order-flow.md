...
Updated 19 September 2026. The agreed shortage, confirmation, packing, delivery return, credit, refund, collection, message retry and final-closure flows are implemented. The current operating guide is [the updated flow PDF](../output/pdf/aapoorti-b-connect-whatsapp-order-flow-updated.pdf). Earlier specification sections below provide design context; later implementation sections record the delivered behavior.
...
- An amount paid above the revised bill remains visible as Financial Review. The retailer-finance workflow below now reconciles verified excess as ledger credit or a requested refund; the packing review finalizes after reconciliation.
...
- Physical receipt releases the trip's return-custody hold. The return case closes after verified settlement or documented retailer ledger credit, with no unverified receipts or unresolved refund. Pending collection, payment verification and unresolved refunds remain Warehouse Received for financial follow-up; later reconciliation updates status and its audit/outbox.
...
- Closure checks run independently from outbound retries, so a Meta outage cannot block reconciliation. Existing created orders are rechecked, and the order card lists the reasons still open. Manual ?Completed? notifications are rejected when closure checks fail. The combined all-age Admin queue (item 11) is now implemented as described below.
